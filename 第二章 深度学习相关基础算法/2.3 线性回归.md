...
$$D={x_{i,j}, y_{i}}$$，其中i,表示数据集第i位序列的值，而j则表示在第i位x的属性是j种属性，即数据集中，x(i,j)，i维行，j维纵坐标的矩阵。
...
这里考虑一元的情况则数据集为： $$D=\left [ x_{i}, y_{i}\right ]_{i=1}^{m} $$,即上面矩阵表示的j元数据集中j=1，

## 2.2 多元线性回归

** 上式4 求最小值时的参数a,b成为线性回归模型的最小二乘法“参数估计”（parameter estimation）**。